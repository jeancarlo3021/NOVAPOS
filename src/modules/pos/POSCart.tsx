...
export const POSCartPanel: React.FC<POSCartPanelProps> = ({
  ...
  return (
    <div className="w-105 shrink-0 flex flex-col bg-white border-l-2 border-gray-200">
      ...
      {/* ── Header ── */}
      <div className="px-6 py-5 border-b border-gray-200 flex items-center gap-3 bg-white shrink-0">
        <ShoppingBag size={28} className="text-emerald-500" />
        <h2 className="text-gray-900 font-black text-2xl">Carrito</h2>
        {cartItems.length > 0 && (
          <span className="ml-auto bg-emerald-500 text-white text-base font-black w-10 h-10 rounded-full flex items-center justify-center">
            {cartItems.length}
          </span>
        )}
      </div>
        ...
        {cartItems.length === 0 ? (
          ...
          <ul className="divide-y divide-gray-100 px-5 py-2">
            {cartItems.map((item) => {
              ...
              return (
              <li key={item.product_id} className="py-5">
                {/* Name row */}
                <div className="flex items-start gap-3 mb-3">
                  <div className="flex-1 min-w-0">
                    <span className="text-gray-900 text-lg font-black leading-snug">
                      {item.product.name}
                    </span>
                    {/* Promo badge */}
                    {hasPromo && (
                      <span className="ml-2 inline-flex items-center gap-1 text-sm bg-violet-100 text-violet-700 font-bold px-2 py-0.5 rounded-lg">
                        <Tag size={12} /> {item.promo!.type === '2x1' ? '2×1' : item.promo!.type === 'percentage' ? `${item.promo!.value}% desc.` : `-₡${item.promo!.value}`}
                      </span>
                    )}
                  </div>
                  <button
                    onPointerDown={() => onRemoveFromCart(item.product_id)}
                    className="w-12 h-12 rounded-xl bg-red-50 hover:bg-red-100 active:bg-red-200 text-red-500 flex items-center justify-center transition shrink-0"
                  >
                    <Trash2 size={20} />
                  </button>
                </div>
                ...
                <div className="flex items-center justify-between gap-3">
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className="w-14 h-14 rounded-xl bg-gray-100 hover:bg-gray-200 active:bg-gray-300 active:scale-90 text-gray-700 flex items-center justify-center transition"
                    >
                      <Minus size={24} />
                    </button>
                    <span className="w-14 text-center text-gray-900 font-black text-3xl select-none">
                      {item.quantity}
                    </span>
                    <button
                      onPointerDown={() => onChangeQuantity(item.product_id, item.quantity + 1)}
                      className="w-14 h-14 rounded-xl bg-emerald-500 hover:bg-emerald-600 active:bg-emerald-700 active:scale-90 text-white flex items-center justify-center transition"
                    >
                      <Plus size={24} />
                    </button>
                  </div>
                  ...
                  <div className="text-right">
                    ...
                    {showOriginal && (
                      <p className="text-gray-300 text-base line-through">
                        ₡{originalSubtotal.toLocaleString()}
                      </p>
                    )}
                    <p className={`font-black text-2xl ${showOriginal ? 'text-violet-600' : 'text-emerald-600'}`}>
      ...
      {/* ── Totals ── */}
      <div className="border-t-2 border-gray-100 px-6 py-5 bg-gray-50 shrink-0 space-y-3">
            ...
            return (
              <>
                <div className="flex justify-between items-center">
                  <span className="text-gray-400 text-base font-medium">Precio original</span>
                  <span className="text-gray-400 text-base line-through">₡{grossSubtotal.toLocaleString()}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-violet-600 text-base font-semibold">Descuentos / Promos</span>
                  <span className="text-violet-600 text-base font-semibold">-₡{totalDiscount.toLocaleString()}</span>
                </div>
              </>
            );
          }
        ...
        <div className="flex justify-between items-center">
          ...
          <span className="text-gray-800 text-lg font-bold">₡{subtotal.toLocaleString()}</span>
        </div>
        {taxEnabled && taxAmount > 0 && (
          <div className="flex justify-between items-center">
            ...
            <span className="text-gray-800 text-lg font-bold">₡{taxAmount.toLocaleString()}</span>
          </div>
        )}
        <div className="flex justify-between items-center pt-3 border-t-2 border-gray-200">
          <span className="text-gray-900 font-black text-2xl">Total</span>
          <span className="text-emerald-600 font-black text-4xl">₡{total.toLocaleString()}</span>
        </div>
      </div>

      {/* ── Pay button ── */}
      <div className="px-5 py-5 bg-white border-t border-gray-200 shrink-0">
        {!currentSession ? (
          ...
          <div className="bg-red-50 border-2 border-red-200 rounded-2xl px-5 py-4 mb-4 text-center">
        ...
        <button
          ...
          disabled={!canPay}
          className={`
            w-full h-24 flex items-center justify-center gap-3 font-black text-3xl rounded-2xl transition
            ${canPay
